...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "A:t:c:u:g:m:n:r:J:w:x:l:hdaijp:sz:P:D:T:IU:", ["hook=", "auto", "autooff"])
    ...
    try:
        zap = ZAPv2(proxies={'http': 'http://' + zap_ip + ':' + str(port), 'https': 'http://' + zap_ip + ':' + str(port)}, apikey=zap_apikey)
